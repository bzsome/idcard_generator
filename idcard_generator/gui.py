...
class IDGen:
    ...
    def generator(self):
        ...
        if self.eBgvar.get():
            ...
            empty_image = cv2.cvtColor(np.asarray(empty_image), cv2.COLOR_RGBA2BGRA)
    ...
    def show_ui(self, root):
        ...
        link.grid(row=0, column=2, sticky=W, padx=26, pady=3, columnspan=4)
        link.bind("<Button-1>", utils.open_url)

        Label(root, text='姓名:').grid(row=1, column=0, sticky=W, padx=3, pady=3)
        self.eName = Entry(root, width=8)
        self.eName.grid(row=1, column=1, sticky=W, padx=3, pady=3)
        Label(root, text='性别:').grid(row=1, column=2, sticky=W, padx=3, pady=3)
        self.eSex = Entry(root, width=8)
        self.eSex.grid(row=1, column=3, sticky=W, padx=3, pady=3)
        Label(root, text='民族:').grid(row=1, column=4, sticky=W, padx=3, pady=3)
        self.eNation = Entry(root, width=8)
        self.eNation.grid(row=1, column=5, sticky=W, padx=3, pady=3)
        Label(root, text='出生年:').grid(row=2, column=0, sticky=W, padx=3, pady=3)
        self.eYear = Entry(root, width=8)
        self.eYear.grid(row=2, column=1, sticky=W, padx=3, pady=3)
        Label(root, text='月:').grid(row=2, column=2, sticky=W, padx=3, pady=3)
        self.eMon = Entry(root, width=8)
        self.eMon.grid(row=2, column=3, sticky=W, padx=3, pady=3)
        Label(root, text='日:').grid(row=2, column=4, sticky=W, padx=3, pady=3)
        self.eDay = Entry(root, width=8)
        self.eDay.grid(row=2, column=5, sticky=W, padx=3, pady=3)
        Label(root, text='住址:').grid(row=3, column=0, sticky=W, padx=3, pady=3)
        self.eAddr = Entry(root, width=32)
        self.eAddr.grid(row=3, column=1, sticky=W, padx=3, pady=3, columnspan=5)
        Label(root, text='证件号码:').grid(row=4, column=0, sticky=W, padx=3, pady=3)
        self.eIdn = Entry(root, width=32)
        self.eIdn.grid(row=4, column=1, sticky=W, padx=3, pady=3, columnspan=5)
        Label(root, text='签发机关:').grid(row=5, column=0, sticky=W, padx=3, pady=3)
        self.eOrg = Entry(root, width=32)
        self.eOrg.grid(row=5, column=1, sticky=W, padx=3, pady=3, columnspan=5)
        Label(root, text='有效期限:').grid(row=6, column=0, sticky=W, padx=3, pady=3)
        self.eLife = Entry(root, width=32)
        self.eLife.grid(row=6, column=1, sticky=W, padx=3, pady=3, columnspan=5)
        Label(root, text='选项:').grid(row=7, column=0, sticky=W, padx=3, pady=3)
        self.eBgvar = IntVar()
        self.ebg = Checkbutton(root, text='自动抠图', variable=self.eBgvar)
        self.ebg.grid(row=7, column=1, sticky=W, padx=3, pady=3, columnspan=5)

        random_btn = Button(root, text='随机', width=8, command=self.random_data)
        random_btn.grid(row=8, column=0, sticky=W, padx=16, pady=3, columnspan=2)
        generator_btn = Button(root, text='选择头像并生成', width=24, command=self.generator)
        generator_btn.grid(row=8, column=2, sticky=W, padx=1, pady=3, columnspan=4)
